...
const resolveSigningConfig = (
  ...
  const envKeyId = Env.get('POSTGRES_PROXY_KEY_ID', appName);
  const envSecret = Env.get('POSTGRES_PROXY_SECRET', appKey);
  ...
  const requireSigning =
    overrides.requireSigning ?? Env.getBool('POSTGRES_PROXY_REQUIRE_SIGNING', true);
  const signingWindowMs =
    overrides.signingWindowMs ?? Env.getInt('POSTGRES_PROXY_SIGNING_WINDOW_MS', 60000);
