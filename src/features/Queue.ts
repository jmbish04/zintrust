...
export const RedisQueue = Object.freeze({
  ...
  async process(_handler: (job: QueueJob) => Promise<void>): Promise<void> {
    Logger.warn(
      '[RedisQueue] Manual processing not recommended for BullMQ. Use proper workers instead.'
    );
  },
});
...
export const Queue = (() => {
  ...
  switch (driver) {
    ...
    case 'database':
      ...
      Logger.warn(
        `Queue driver '${driver}' not implemented in this template. Falling back to in-memory.`
      );
