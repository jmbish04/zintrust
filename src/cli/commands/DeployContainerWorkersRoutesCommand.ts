...
type DeployCwrOptions = CommandOptions & {
  noBuild?: boolean;
  removeOrphans?: boolean;
};

const resolveComposePath = (): string => {
  const composePath = join(process.cwd(), 'docker-compose.workers-routes.yml');
  if (!existsSync(composePath)) {
    throw ErrorFactory.createCliError(
      'docker-compose.workers-routes.yml not found. Run `zin init:cwr` first.'
    );
  }
  return composePath;
};
...
export const DeployContainerWorkersRoutesCommand = Object.freeze({
  create(): IBaseCommand {
    return BaseCommand.create({
      name: 'deploy:cwr',
      aliases: ['deploy:container-workers-routes'],
      description: 'Deploy Workers Routes container stack (docker-compose.workers-routes.yml)',
      ...
      execute: async (options: DeployCwrOptions): Promise<void> => {
        ...
        Logger.info('Deploying container workers routes stack...');
        await runCompose(args);
        Logger.info('✅ Container workers routes deployed.');
      },
    });
  },
});
