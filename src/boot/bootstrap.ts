...
const BootstrapFunctions = Object.freeze({
  ...
  async start(): Promise<void> {
    let currentStep = 'start:init';

    try {
      Logger.debug('[bootstrap] step: start:init');
      ...
      try {
        currentStep = 'plugins:auto-imports:load';
        Logger.debug('[bootstrap] step: plugins:auto-imports:load');
        const { PluginAutoImports } = await import('@runtime/PluginAutoImports');

        currentStep = 'plugins:auto-imports:run';
        Logger.debug('[bootstrap] step: plugins:auto-imports:run');
        await PluginAutoImports.tryImportProjectAutoImports();

        Logger.debug('[bootstrap] step: plugins:auto-imports:done');
      } catch (error) {
      ...
      // }
      currentStep = 'app:create';
      Logger.debug('[bootstrap] step: app:create');
      const app = Application.create(Env.ZINTRUST_PROJECT_ROOT || undefined);
      appInstance = app;

      // Boot application
      currentStep = 'app:boot';
      Logger.debug('[bootstrap] step: app:boot');
      await app.boot();

      // Get port and host from environment
      currentStep = 'server:resolve-config';
      Logger.debug('[bootstrap] step: server:resolve-config');
      const port = Env.getInt('PORT', 7777);
      const host = Env.get('HOST', 'localhost');

      // Create and start server
      currentStep = 'server:create';
      Logger.debug('[bootstrap] step: server:create', { host, port });
      const server = Server.create(app, port, host);
      serverInstance = server;

      // Start listening
      currentStep = 'server:listen';
      Logger.debug('[bootstrap] step: server:listen');
      await server.listen();
      ...
      currentStep = 'schedules:start';
      Logger.debug('[bootstrap] step: schedules:start');
      ...
      if (
        ...
        currentStep = 'workers:start';
        Logger.debug('[bootstrap] step: workers:start');
        await useWorkerStarter();
      }

      currentStep = 'start:done';
      Logger.debug('[bootstrap] step: start:done');
    } catch (error) {
      try {
