...
const BootstrapFunctions = Object.freeze({
  ...
  async start(): Promise<void> {
    try {
      Logger.debug('[bootstrap] start: begin');
      ...
      try {
        Logger.debug('[bootstrap] plugins: loading project auto-imports');
        ...
        Logger.debug('[bootstrap] plugins: load result', result);
      } catch (error) {
      ...
      // }
      Logger.debug('[bootstrap] app: creating application');
      const app = Application.create(Env.ZINTRUST_PROJECT_ROOT || undefined);
      appInstance = app;

      // Boot application
      Logger.debug('[bootstrap] app: booting');
      await app.boot();
      Logger.debug('[bootstrap] app: boot complete');

      // Get port and host from environment
      Logger.debug('[bootstrap] server: resolve host/port');
      const port = Env.getInt('PORT', 7777);
      const host = Env.get('HOST', 'localhost');

      // Create and start server
      Logger.debug('[bootstrap] server: creating');
      const server = Server.create(app, port, host);
      serverInstance = server;

      // Start listening
      Logger.debug('[bootstrap] server: listening');
      await server.listen();
      Logger.debug('[bootstrap] server: listening started');
      ...
      Logger.debug('[bootstrap] schedules: starting (if needed)');
      await startSchedulesIfNeeded(app);
      Logger.debug('[bootstrap] schedules: done');

      if (appConfig.detectRuntime() === 'nodejs' || appConfig.detectRuntime() === 'lambda') {
        Logger.debug('[bootstrap] workers: initialize (if supported)');
        await useWorkerStarter();
        Logger.debug('[bootstrap] workers: init complete');
      }
      Logger.debug('[bootstrap] start: complete');
    } catch (error) {
