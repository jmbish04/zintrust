...
Cloudflare KV is a Workers binding (not a Redis-like server you can connect to directly). If your app runs **outside Cloudflare** but you still want KV semantics (low-latency key-value), ZinTrustsupports a secure pattern:
...
Default binding name expected by ZinTrustis `CACHE`.
