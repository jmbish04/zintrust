...
ZinTrustfeatures a robust, file-based logging system that helps you monitor your application and debug issues.
...
ZinTrustsupports standard log levels:
...
ZinTrustautomatically rotates log files daily or when they reach a certain size (default 10MB), keeping your disk space usage under control.
...
ZinTrustenforces a "Zero-Swallow" safety guarantee: all errors must be logged before being handled or re-thrown.
...
ZinTrustcan optionally forward logs to cloud backends in addition to console/file output. These backends are **best-effort** and designed to be non-blocking.
